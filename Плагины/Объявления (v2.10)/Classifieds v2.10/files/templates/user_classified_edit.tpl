...
<tr>
<td class='tab0'>&nbsp;</td>
<td class='tab1' NOWRAP><a href='user_classified.php'>{$user_classified_edit1}</a></td>
<td class='tab'>&nbsp;</td>
<td class='tab2' NOWRAP><a href='user_classified_settings.php'>{$user_classified_edit2}</a></td>
<td class='tab'>&nbsp;</td>
<td class='tab2' NOWRAP><a href='user_classified_browse.php'>{$user_classified_edit7}</a></td>
<td class='tab3'>&nbsp;</td>
</tr>
</table>

<img src='./images/icons/classified48.gif' border='0' class='icon_big'>
<div class='page_header'>{$user_classified_edit3}</div>
<div>{$user_classified_edit4}</div>
...
<form action='user_classified_edit.php' method='post' name='info' id="classifiedform">
<table cellpadding='0' cellspacing='0'>
<tr>
<td class='form1' style='width: 90px;'>{$user_classified_edit5}</td>
...
{if $classifiedcats|@count > 0}
  <tr>
  <td class='form1'>{$user_classified_edit6}</td>
  <td class='form2'>
       ...
       {if $classifiedcats[classifiedcat_loop2].classifiedcat_subcats|@count > 0}
         <select id='subcats_{$classifiedcats[classifiedcat_loop2].classifiedcat_id}' onChange="setCatId('subcats_{$classifiedcats[classifiedcat_loop2].classifiedcat_id}');">
           ...
           {section name=classifiedsubcat_loop loop=$classifiedcats[classifiedcat_loop2].classifiedcat_subcats}
             <option value='{$classifiedcats[classifiedcat_loop2].classifiedcat_subcats[classifiedsubcat_loop].classifiedsubcat_id}'{if $classifiedcats[classifiedcat_loop2].classifiedcat_subcats[classifiedsubcat_loop].classifiedsubcat_id == $classified_classifiedsubcat_id} selected='selected'{/if}>{$classifiedcats[classifiedcat_loop2].classifiedcat_subcats[classifiedsubcat_loop].classifiedsubcat_title}</option>
           {/section}
         </select>
       {/if}
     </div>
...
<tr>
<td class='form1' style='width: 90px;'>{$user_classified_edit18}</td>
<td class='form2'>
  ...
  {if $privacy_options|@count > 1 OR $comment_options|@count > 1}
    <div id='settings_show' class='classified_settings_link'>
      <a href="javascript:showdiv('entry_settings');hidediv('settings_show');">{$user_classified_edit8}</a>
    </div>
  {/if}

  <div id='entry_settings' class='classified_settings' style='display: none;'>
    {* SHOW SEARCH PRIVACY OPTIONS IF ALLOWED BY ADMIN *}
    {if $search_classifieds == 1}
      <b>{$user_classified_edit10}</b>
      <table cellpadding='0' cellspacing='0'>
        <tr><td><input type='radio' name='classified_search' id='classified_search_1' value='1'{if $classified_search == 1} CHECKED{/if}></td><td><label for='classified_search_1'>{$user_classified_edit11}</label></td></tr>
        <tr><td><input type='radio' name='classified_search' id='classified_search_0' value='0'{if $classified_search == 0} CHECKED{/if}></td><td><label for='classified_search_0'>{$user_classified_edit12}</label></td></tr>
      </table>
    {/if}

    {* ADD SPACE IF BOTH OPTIONS ARE AVAILABLE *}
    {if $search_classifieds == 1 AND ($privacy_options|@count > 1 OR $comment_options|@count > 1)}<br>{/if}

    {* SHOW PRIVACY OPTIONS IF ALLOWED BY ADMIN *}
    {if $privacy_options|@count > 1}
      <b>{$user_classified_edit13}</b>
      <table cellpadding='0' cellspacing='0'>
    ...
    {if $comment_options|@count > 1}
      <b>{$user_classified_edit14}</b>
      <table cellpadding='0' cellspacing='0'>
        {section name=comment_loop loop=$comment_options}
          <tr><td><input type='radio' name='classified_comments' id='{$comment_options[comment_loop].comment_id}' value='{$comment_options[comment_loop].comment_value}'{if $classified_comments == $comment_options[comment_loop].comment_value} CHECKED{/if}></td><td><label for='{$comment_options[comment_loop].comment_id}'>{$comment_options[comment_loop].comment_option}</label></td></tr>
        {/section}
      </table>
    {/if}
  </div>
  ...
  <tr>
  <td>
    <input type='submit' class='button' value='{$user_classified_edit15}'>&nbsp;
  ...
  <td>
    ...
    <input type='submit' class='button' value='{$user_classified_edit17}'>
    </form>
  </td>
  </tr>
  </table>
