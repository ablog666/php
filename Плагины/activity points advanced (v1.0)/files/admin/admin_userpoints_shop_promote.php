...
if($task == "addoffer") {
  ...
  for(;;) {
    ...
    if( !array_key_exists( $promotion_type, $promotion_classes ) ) {
      $error_message = $admin_userpoints_shop_promote[46];
      break;
    }
      
    if(empty($offer_title)) {
    $error_message = $admin_userpoints_shop_promote[45];
      break;
  }
 
    if(!class_exists( $promotion_classes[$promotion_type] ) ) {
      $error_message = $admin_userpoints_shop_promote[$promotion_type];
      break;
    }
