...
function upearner_votepoll_ontransactionsuccess( $params ) {
  ...
  if( !$owner_username ) {
    $params['err_msg'] = $functions_userpoints[41];
    return false;
  }

  $params['transaction_text'] = $functions_userpoints[40] . " <a href='" . $url->url_create('poll', $owner_username, $poll_id) . "'>{$functions_userpoints[42]}</a>";
