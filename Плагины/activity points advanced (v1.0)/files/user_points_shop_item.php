...
if($task == "dobuy") {
  ...
  if(!$upspender->transact( $user ) ) {
	...
	$transaction_message = !empty($upspender->transaction_message) ? $upspender->transaction_message :  "You successfully bought this item.";
...
if(($task == "dopost") && ($allowed_to_comment != 0)) {
  ...
  if($is_error == 0) {
    ...
    if($user->user_exists != 0) {
      ...
      // action ? 
      // $actions->actions_add($user, "upcomment", Array('[username1]', '[username2]', '[id]', '[comment]'), Array($commenter, $owner->user_info[user_username], $item_id, $comment_body_encoded));
    }
    //else {
    //  $commenter = "Anonymous";
    //}
...
if($task != "buy") {
  ...
  $upspender->upspender_info['userpointspender_body'] = str_replace("\r\n", "<br>", html_entity_decode( $upspender->upspender_info['userpointspender_body'] ));
