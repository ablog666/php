...
if($task == "dobuy") {
  ...
  if(!$upearner->transact( $user ) ) {
	...
	$transaction_message = !empty($upearner->transaction_message) ? $upearner->transaction_message :  $user_points_offers_item[22];
...
if(($task == "dopost") && ($allowed_to_comment != 0)) {
  ...
  if($is_error == 0) {
    ...
    if($user->user_exists != 0) {
      ...
      // action ? 
      // $actions->actions_add($user, "upitemcomment", Array('[username1]', '[username2]', '[id]', '[comment]'), Array($commenter, $owner->user_info[user_username], $item_id, $comment_body_encoded));
    }
...
if($task != "buy") {
  ...
  $upearner->upearner_info['userpointearner_body'] = str_replace("\r\n", "<br>", html_entity_decode( $upearner->upearner_info['userpointearner_body'] ));
