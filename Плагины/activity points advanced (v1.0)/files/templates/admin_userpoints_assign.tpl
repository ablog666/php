...
<h2>{$admin_userpoints_assign1}</h2>
{$admin_userpoints_assign2}
...
{if $result != 0}
  ...
  {if empty($error)}
  <div class='success'><img src='../images/success.gif' class='icon' border='0'> {$admin_userpoints_assign6}</div>
  {else}
...
<form action='admin_userpoints_assign.php' method='POST'>
  ...
  <tr>
  <td class='header' Xwidth='200' Xstyle='padding-left: 0px;'>{$admin_userpoints_assign3}</td>
  <td class='header'>{$admin_userpoints_assign4}</td>
  <td class='header'>{$admin_userpoints_assign8}</td>
  <td class='header'>{$admin_userpoints_assign9}</td>
  </tr>
  ...
  {/if}
    ...
    <td class='item' Xstyle='padding-left: 0px;' Xwidth="100%">  <div id='upedit_{$smarty.section.action_loop.index}{$smarty.section.action_gloop.index}' style="display:none;width:260px"><input id='upinput_{$smarty.section.action_loop.index}{$smarty.section.action_gloop.index}' {if $unavailable}disabled{/if} type='text' class='text' size=40 name='actionsname[{$actions[action_loop][action_gloop].action_id}{$actions[action_loop][action_gloop].actiontype_name}]' value='{$actions[action_loop][action_gloop].action_name}'></div>  <div id='upshow_{$smarty.section.action_loop.index}{$smarty.section.action_gloop.index}' style="display:block;width:260px" {if !$unavailable}onclick="up_edit_inplace('{$smarty.section.action_loop.index}{$smarty.section.action_gloop.index}')"{/if}> {$actions[action_loop][action_gloop].action_name}&nbsp;{if $unavailable}<br><font color="red"> {$admin_userpoints_assign7} {$actions[action_loop][action_gloop].action_requiredplugin}</font> {/if} </div></td>
    ...
    <td class='item' width="100px"><input {if $unavailable}disabled{/if}  type='text' class='text' size=5 name='actionsrollover[{$actions[action_loop][action_gloop].action_id}{$actions[action_loop][action_gloop].actiontype_name}]' value='{$actions[action_loop][action_gloop].action_rolloverperiod}'> {$admin_userpoints_assign10}</td>
    </tr>
  {/section}
...
<input type='submit' class='button' value='{$admin_userpoints_assign5}'>
