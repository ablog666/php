...
function addText(commentBody) {
  if(commentBody.value == '') {
    commentBody.value = '{/literal}{$user_points_shop_item14}{literal}';
...
function checkText() {
  ...
  commentSubmit.value = '{/literal}{$user_points_shop_item15}{literal}';
...
function addComment(is_error, comment_body, comment_date) {
  if(is_error == 1) {
    ...
    if(comment_body == '') {
      commentError.innerHTML = '{/literal}{$user_points_shop_item16}{literal}';
    } else {
      commentError.innerHTML = '{/literal}{$user_points_shop_item17}{literal}';
    }
    var commentSubmit = document.getElementById('comment_submit');
    commentSubmit.value = '{/literal}{$user_points_shop_item18}{literal}';
    ...
    {/literal}
      {if $user->user_info.user_id != 0}
        newTable += "<a href='{$url->url_create('profile',$user->user_info.user_username)}'><img src='{$user->user_photo('./images/nophoto.gif')}' class='photo' border='0' width='{$misc->photo_size($user->user_photo('./images/nophoto.gif'),'150','150','w')}'></a></td><td class='userpoints_item2'><table cellpadding='0' cellspacing='0' width='100%'><tr><td class='userpoints_comment_author'><b><a href='{$url->url_create('profile',$user->user_info.user_username)}'>{$user->user_info.user_username}</a></b> - {$datetime->cdate("`$setting.setting_timeformat` `$user_points_shop_item30` `$setting.setting_dateformat`", $datetime->timezone($smarty.now, $global_timezone))}</td><td class='userpoints_comment_author' align='right' nowrap='nowrap'>&nbsp;[ <a href='user_messages_new.php?to={$user->user_info.user_username}'>{$user_points_shop_item20}</a> ]</td>";
      {else}
        newTable += "<img src='./images/nophoto.gif' class='photo' border='0' width='75'></td><td class='userpoints_item2'><table cellpadding='0' cellspacing='0' width='100%'><tr><td class='userpoints_comment_author'><b>{$user_points_shop_item11}</b> - {$datetime->cdate("`$setting.setting_timeformat` `$user_points_shop_item30` `$setting.setting_dateformat`", $datetime->timezone($smarty.now, $global_timezone))}</td><td class='userpoints_comment_author' align='right' nowrap='nowrap'>&nbsp;</td>";
      {/if}
...
{* SHOW ITEM *}
  ...
  <td valign='top'>
	...
	<input id="subscribe_now" type="submit" id="submit_button" name="submit_button" value="{$user_points_shop_item22}" />
  ...
  {/if}
	  ...
	  <div style='color: #888888;'>
		...
		{$user_points_shop_item6}&nbsp;{$datetime->cdate("`$setting.setting_dateformat`", $datetime->timezone($upspender_date, $global_timezone))}, 
		{$upspender->upspender_info.userpointspender_comments} {$user_points_shop_item5},
		{$upspender->upspender_info.userpointspender_views} {$user_points_shop_item4}
	  </div>
	<div style='padding-top: 10px;'>
	  {$upspender->upspender_info.userpointspender_body}
	  <br><br>
	  {$user_points_shop_item3}&nbsp;<strong>{$upspender->upspender_info.userpointspender_cost}</strong>&nbsp;{$user_points_shop_item21}
...
<div>
  <a href='./user_points_shop.php'><img src='./images/icons/back16.gif' border='0' class='icon'>{$user_points_shop_item7}</a>
</div>
  ...
  <tr>  
  <td class='header'>
	{$user_points_shop_item8} (<span id='total_comments'>{$total_comments}</span>)
  </td>
  </tr>
  {if $allowed_to_comment != 0}
	...
	<td class='userpoints_postcomment'>
	  ...
	  <textarea name='comment_body' id='comment_body' rows='2' cols='65' onfocus='removeText(this)' onblur='addText(this)' style='color: #888888; width: 100%;'>{$user_points_shop_item14}</textarea>
	  ...
	  {if $setting.setting_comment_code == 1}
		...
		<td width='10'><img src='./images/icons/tip.gif' border='0' class='icon' onMouseover="tip('{$user_points_shop_item19}')"; onMouseout="hidetip()"></td>
	  {/if}
	  <td align='right' style='padding-top: 5px;'>
	  <input type='submit' id='comment_submit' class='button' value='{$user_points_shop_item18}'>
	  <input type='hidden' name='shopitem_id' value='{$upspender->upspender_info.userpointspender_id}'>
  ...
  <td class='userpoints' id='userpoints_comments'>
	...
	{section name=comment_loop loop=$comments}
	  ...
	  <td class='userpoints_item2'>
		<table cellpadding='0' cellspacing='0' width='100%'>
		<tr>
		<td class='userpoints_comment_author'><b>{if $comments[comment_loop].comment_author->user_info.user_id != 0}<a href='{$url->url_create('profile',$comments[comment_loop].comment_author->user_info.user_username)}'>{$comments[comment_loop].comment_author->user_info.user_username}</a>{else}{$user_points_shop_item11}{/if}</b> - {$datetime->cdate("`$setting.setting_timeformat` `$user_points_shop_item30` `$setting.setting_dateformat`", $datetime->timezone($comments[comment_loop].comment_date, $global_timezone))}</td>
		<td class='userpoints_comment_author' align='right' nowrap='nowrap'>&nbsp;[ <a href='user_messages_new.php?to={$comments[comment_loop].comment_author->user_info.user_username}'>{$user_points_shop_item20}</a> ]</td>
		</tr>
		<tr>
